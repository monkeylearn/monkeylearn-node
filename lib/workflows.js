...
function Workflows(ml) {
  ...
  this.metadata = new WorkflowMetadata(this);
}
...
function WorkflowMetadata(workflows) {
  this.ml = workflows.ml;
  this.workflows = workflows;
}

WorkflowMetadata.prototype.create = function(model_id, params) {
  return new Promise((resolve, reject) => {
    if (!isString(model_id)) {
      throw new MonkeyLearnError('', null, 'A model_id string is required.');
    }
  ...
  .then(_ =>
    request(this.ml, {
      url: `${this.workflows.base_url}${model_id}/metadata/`,
      method: 'POST',
      body: params
    })
  )
